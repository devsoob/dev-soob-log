  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black flex flex-col">
      ...
      <article className="flex-1 max-w-4xl mx-auto px-4 xs:px-6 sm:px-10 pt-20 xs:pt-24 pb-8 w-full">
        <Link href="/" className="text-blue-500 hover:text-blue-600 mb-4 inline-block text-base xs:text-lg sm:text-xl">
          ← Back to Home
        </Link>
        ...
        <div className="flex flex-col gap-2 xs:gap-3 text-black dark:text-white">
          ...
          {post.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 xs:gap-3 mt-2">
              {post.tags.map(tag => (
                <span key={tag} className="bg-white dark:bg-black border border-black dark:border-white px-2 py-1 rounded-md text-xs xs:text-sm">
                  {tag}
                </span>
              ))}
            </div>
