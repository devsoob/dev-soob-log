...
export default function Header() {
  ...
  return (
    <header className="fixed w-full top-0 z-50 bg-white dark:bg-[#1a1a1a] border-b border-gray-200 dark:border-gray-800 bg-opacity-80 dark:bg-opacity-80 backdrop-blur-sm">
      <div className={`mx-auto px-4 sm:px-6 h-16 sm:h-16 md:h-16 flex items-center justify-between ${isHomePage ? 'max-w-7xl lg:px-8' : isPostPage ? 'max-w-6xl lg:px-8' : 'max-w-4xl lg:px-0'}`}>
        <Link href="/" className="text-xl xs:text-2xl font-bold italic hover:scale-105 transition-transform duration-200 text-black dark:text-white px-3 py-2 min-h-[44px] min-w-[44px] flex items-center" aria-label="Dev Soob Log - Home">
          Dev Soob Log
        </Link>
        <nav className="flex items-center space-x-1 xs:space-x-2">
          ...
          >
            <RssIcon className="text-black dark:text-white w-5 h-5" aria-hidden="true" />
          </Link>
        </nav>
      </div>
    </header>
